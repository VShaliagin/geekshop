...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        action_2_condition = Q(order__updated__gt=F('order_created') + action_1_time_delta) & \
                             Q(order__updated__lte=F('order_created') + action_2_time_delta)
        action_expired_condition = Q(order__updated__ht=F('order_created') + action_2_time_delta)
        ...
        for orderitem in order_discounts:
            print(f'{orderitem.action_oredr:2}: order №{orderitem.pk:3}: {orderitem.product.name:15}: '
                  f'discount {orderitem.total_price:10} | {orderitem.order.updated - orderitem.order.created}')